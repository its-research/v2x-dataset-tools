...
def vis_label_in_image(path, save_path):
    ...
    for data_info in path_data_infos:
        ...
        if osp.basename(path) in [
                'infrastructure-side', 'single-infrastructure-side'
        ]:
        ...
        for ori in oris:
            if 'rotation' not in ori.keys():
                ori['rotation'] = 0.0
            labels.append(
                [ori['3d_dimensions'], ori['3d_location'], ori['rotation']])

        cam_instrinsic_path = osp.join(
            path,
            data_info['calib_camera_intrinsic_path'])  # intrinsic pf camera
        ...
        vis_label_in_img(camera_8_points_list, image_path, cam_instrinsic_path,
                         save_path)
...
def add_arguments(parser):
    parser.add_argument(
        '--path',
        type=str,
        default='./datasets/cooperative-vehicle-infrastructure/vehicle-side',
    )
    parser.add_argument('--id', type=int, default=0)
    parser.add_argument('--output-file', type=str, default='./outputs/veh')
