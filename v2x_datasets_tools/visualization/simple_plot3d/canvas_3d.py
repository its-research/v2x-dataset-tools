...
Simple 3D visualization for 3D points & boxes. Intended as a simple, hackable alternative to mayavi for certain point cloud tasks.
"""
...
class Canvas_3D(object):
    ...
    def __init__(
        ...
        self.ext_matrix = self.get_extrinsic_matrix(self.camera_center_coords,
                                                    self.camera_focus_coords)
    ...
    def draw_boxes(
        ...
        for i, (color, curr_corners_xy, curr_valid_mask) in enumerate(
                zip(colors.tolist(), corners_xy.tolist(),
                    valid_mask.tolist())):
    ...
    def get_extrinsic_matrix(
        ...
        Thoughts:
            Remember that in camera coordinates, pos x is right, pos y is up,
                pos z is forward.
        """
        ...
        az, el, depth = Canvas_3D.cart2sph(
            np.array(
                [[focus_x - center_x, focus_y - center_y,
                  focus_z - center_z]]))
        az = float(az)
        el = float(el)
        depth = float(depth)
        ...
        z_rot = np.array([[np.cos(az), -np.sin(az), 0],
        ...
        # Now, how the z_rot and y_rot work (spherical coordinates), is it
