...
from v2x_datasets_tools.utils import box_utils, common_utils
...
def color_encoding(intensity, mode="intensity"):
    ...
    if mode == "intensity":
        ...
        int_color = np.c_[
            np.interp(intensity_col, VID_RANGE, VIRIDIS[:, 0]), np.interp(intensity_col, VID_RANGE, VIRIDIS[:, 1]), np.interp(intensity_col, VID_RANGE, VIRIDIS[:, 2])
        ]
...
def draw_points_boxes_plt(pc_range, points=None, boxes_pred=None, boxes_gt=None, save_path=None, points_c="y.", bbox_gt_c="green", bbox_pred_c="red", return_ax=False, ax=None):
    if ax is None:
