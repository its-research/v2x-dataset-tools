...
def project_3d(p2, x3d, y3d, z3d, w3d, h3d, l3d, ry3d, de_norm):
    ...
    R2 = np.array([[1, 0, 0], [0, -de_norm[1], +de_norm[2]],
...
def load_detect_data(filename):
    ...
    with open(filename) as infile:
        ...
        for line in infile:
            ...
            t_data.obj_type = fields[0].lower(
...
def project_3d_ground(p2, de_bottom_center, w3d, h3d, l3d, ry3d, de_norm,
                      c2g_trans):
    """
    Projects a 3D box into 2D vertices using the camera2ground transformation
...
def project_3d_world(p2, de_center_in_world, w3d, h3d, l3d, ry3d,
                     camera2world):
    """
    help with world
    Projects a 3D box into 2D vertices using the camera2world transformation
    Note: Since the roadside camera contains pitch and roll angle w.r.t. the ground/world,
    simply adopting KITTI-style projection not works. We first compute the 3D bounding box in ground-coord and then convert back to camera-coord.
...
def compute_c2g_trans(de_norm):
    ...
    c2g_trans = np.vstack([ground_x_axis, ground_y_axis,
                           ground_z_axis])  # (3, 3)
    return c2g_trans
...
def get_camera_3d_8points_g2c(w3d,
                              h3d,
                              l3d,
                              yaw_ground,
                              center_ground,
    ...
    corners_3d_ground = np.matrix(ground_r) * np.matrix(
        corners_3d_ground) + np.matrix(center_ground)  # [3, 8]

    if g2c_trans.shape[0] == 4:  # world2camera transformation
        ones = np.ones(8).reshape(1, 8).tolist()
        corners_3d_cam = g2c_trans * np.matrix(corners_3d_ground.tolist() +
                                               ones)
...
def show_box_with_roll(name_list, thresh=0.5, projectMethod='Ground'):
    """show 2d box and 3d box yexiaoqing modified.

    # 'Ground': using the ground to camera transformation (denorm: the ground plane equation) # 'World': using the extrinsics (world to camera transformation)
    """
    ...
    for i, name in enumerate(name_list):
        ...
        for result_index in range(len(result)):
            ...
            cam_bottom_center = [t.X, t.Y,
                                 t.Z]  # bottom center in Camera coordinate
            if use_extrinsic:
                bottom_center_in_world = (
                    camera2world * np.matrix(cam_bottom_center + [1.0]).T)
                verts3d = project_3d_world(p2, bottom_center_in_world, t.w,
                                           t.h, t.l, t.yaw, camera2world)
            if use_denorm:
