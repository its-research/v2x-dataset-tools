...
def update_nuscenes_infos(pkl_path, out_dir):
    ...
    for i, ori_info_dict in enumerate(mmengine.track_iter_progress(data_list["infos"])):
        ...
        temp_data_info["ego2global"] = convert_quaternion_to_matrix(ori_info_dict["ego2global_rotation"], ori_info_dict["ego2global_translation"])
        ...
        temp_data_info["lidar_points"]["lidar2ego"] = convert_quaternion_to_matrix(ori_info_dict["lidar2ego_rotation"], ori_info_dict["lidar2ego_translation"])
        ...
        for ori_sweep in ori_info_dict["sweeps"]:
            ...
            temp_lidar_sweep["lidar_points"]["lidar2ego"] = convert_quaternion_to_matrix(ori_sweep["sensor2ego_rotation"], ori_sweep["sensor2ego_translation"])
            temp_lidar_sweep["ego2global"] = convert_quaternion_to_matrix(ori_sweep["ego2global_rotation"], ori_sweep["ego2global_translation"])
        ...
        for cam in ori_info_dict["cams"]:
            ...
            empty_img_info["cam2ego"] = convert_quaternion_to_matrix(ori_info_dict["cams"][cam]["sensor2ego_rotation"], ori_info_dict["cams"][cam]["sensor2ego_translation"])
            lidar2sensor = np.eye(4)
...
def update_kitti_infos(pkl_path, out_dir):
    ...
    for ori_info_dict in mmengine.track_iter_progress(data_list):
        ...
        temp_data_info["lidar_points"]["Tr_imu_to_velo"] = ori_info_dict["calib"]["Tr_imu_to_velo"].astype(np.float32).tolist()
...
def update_sunrgbd_infos(pkl_path, out_dir):
    ...
    if out_dir in pkl_path:
        print(f"Warning, you may overwriting " f"the original data {pkl_path}.")
        time.sleep(5)
    METAINFO = {"classes": ("bed", "table", "sofa", "chair", "toilet", "desk", "dresser", "night_stand", "bookshelf", "bathtub")}
    print(f"Reading from input file: {pkl_path}.")
    data_list = mmengine.load(pkl_path)
    print("Start updating:")
    converted_list = []
...
def update_lyft_infos(pkl_path, out_dir):
    ...
    data_list = mmengine.load(pkl_path)
    METAINFO = {
        "classes": ("car", "truck", "bus", "emergency_vehicle", "other_vehicle", "motorcycle", "bicycle", "pedestrian", "animal"),
    }
    print("Start updating:")
    converted_list = []
    for i, ori_info_dict in enumerate(mmengine.track_iter_progress(data_list["infos"])):
...
def update_waymo_infos(pkl_path, out_dir):
    ...
    for ori_info_dict in mmengine.track_iter_progress(data_list):
        ...
        for cam_idx, cam_key in enumerate(camera_types):
            ...
            temp_data_info["images"][cam_key]["lidar2img"] = (ori_info_dict["calib"][f"P{cam_idx}"] @ lidar2cam).tolist()

        # image path
        base_img_path = Path(ori_info_dict["image"]["image_path"]).name

        for _cam_idx, cam_key in enumerate(camera_types):
...
def parse_args():
    ...
    parser.add_argument("--pkl-path", type=str, default="./data/kitti/kitti_infos_train.pkl ", help="specify the root dir of dataset")
    parser.add_argument("--out-dir", type=str, default="converted_annotations", required=False, help="output direction of info pkl")
