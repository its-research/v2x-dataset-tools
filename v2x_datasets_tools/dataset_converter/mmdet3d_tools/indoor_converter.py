...
def create_indoor_info_file(data_path, pkl_prefix="sunrgbd", save_path=None, use_v1=False, workers=4):
    ...
    if pkl_prefix == "scannet":
        ...
        train_dataset = ScanNetSegData(data_root=data_path, ann_file=train_filename, split="train", num_points=8192, label_weight_func=lambda x: 1.0 / np.log(1.2 + x))
        # TODO: do we need to generate on val set?
        val_dataset = ScanNetSegData(data_root=data_path, ann_file=val_filename, split="val", num_points=8192, label_weight_func=lambda x: 1.0 / np.log(1.2 + x))
