...
def vis_label_in_image(path, save_path):
    ...
    for data_info in path_data_infos:
        ...
        if osp.basename(path) in ["infrastructure-side", "single-infrastructure-side"]:
            ...
            lidar2cam_path = osp.join(path, data_info["calib_virtuallidar_to_camera_path"])  # extrinsic of lidar_to_camera
        else:
        ...
        camera_8_points_list = get_cam_8_points(labels, lidar2cam_path)  # Convert the label to the 8 points of the cameara coordinate system
